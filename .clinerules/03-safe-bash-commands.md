...
コマンドを実行する直前、特に `git diff` / `git log` / `git show` / `git branch` / `git blame`、`man`、`less` / `more`、`cp` / `rm` の対話確認、`npm init`、AWS CLI など「ページャー起動」「対話プロンプト」で入力待ちになり得る操作を行う前に使う。

コマンド実行（特に `git commit`）が「Pending」のまま停止する事象を回避・復旧するため、以下を遵守する。
...
3. **非対話実行の徹底**: 意図しない入力待ちを防ぐため、常に非対話フラグ（例: `-y`, `--no-pager`, `--force`）を使用する。
...
`less` / `more` / `view` / 裸の `man` は絶対に使用しない。代わりに非対話の `cat` / `head -n` / `tail -n` を使う（`man` は `man git | cat` のようにパイプする）。
...
- ただし、force push・`reset --hard`・未コミット変更の上書きなどの破壊的操作は、対話プロンプトの回避とは別に、実行前に必ずユーザーへ確認すること。本節はあくまで「意図した操作が対話プロンプトで止まらないようにする」ためのものであり、確認手順を省略する許可ではない。
